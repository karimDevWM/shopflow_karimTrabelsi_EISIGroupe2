...
class TestOrders:
    ...
    def test_commande_avec_coupon(self, client, api_coupon):
        ...
        p = client.post(
            '/products/', json={'name': 'PC', 'price': 100.0, 'stock': 5}).json()
        client.post(f'/cart/?user_id=204',
    ...
    def test_coupon_inexistant_retourne_404(self, client):
        ...
        self._setup_panier(client, user_id=207, price=100.0, quantity=1)

        # 2) Tenter de créer une commande avec un coupon inexistant
        response = client.post("/orders/", json={
            "user_id": 207,
            "coupon_code": "NOPE999"
        })
        # 3) Vérifier le code retour + message
        assert response.status_code == 404
        assert "Coupon non trouvé ou inactif" in response.json()["detail"]

    def test_get_commande_par_id(self, client):
        """Créer une commande puis la récupérer par son id."""
        # 1) Créer une commande
        self._setup_panier(client, user_id=208, price=100.0, quantity=2)
        create_resp = client.post("/orders/", json={"user_id": 208})
        ...
        # 3) Vérifier les données clés
        assert fetched["id"] == order_id
        assert fetched["status"] == "pending"
        assert fetched["total_ttc"] == pytest.approx(
            created["total_ttc"], rel=1e-2)
